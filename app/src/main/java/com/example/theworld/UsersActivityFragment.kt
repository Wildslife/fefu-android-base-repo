...
class UsersActivityFragment : Fragment() {
    ...
    private val adapter = UsersActivityAdapter(object : UsersActivityClick{
        override fun onClicked() {
            val activeFragment = parentFragment?.parentFragmentManager?.findFragmentByTag("firstFragment")
            parentFragment?.parentFragmentManager?.beginTransaction().apply {
                if (activeFragment != null)
                    this?.hide(activeFragment)
                this?.add(R.id.FragCont, UsersActivityDetailsFragment.newInstance(),
                    "fourthFragment"
                )
                this?.addToBackStack("fourthFragment")
                this?.commit()
            }
        }
    })
